...
if (!JsNamespace.exist("Iject.Runtime")) {
    ...
    JsNamespace.declare("Iject.Runtime", {
        ...
        login : function(o)
        {
            ...
            application.mainframe.set_left(nScreenLeft+(nScreenWidth/2)-310);
            application.mainframe.set_top(nScreenTop+(nScreenHeight/2)-240);
            application.mainframe.set_width(Iject.$["loginSize"].width);
            application.mainframe.set_height(Iject.$["loginSize"].height);
            
            application.mainframe.set_titletext("OFFEDUPROJECT.NET 교육시스템");
