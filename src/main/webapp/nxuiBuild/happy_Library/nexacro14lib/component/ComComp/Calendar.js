...
if (!nexacro.Calendar) {
	...
	_pCalendar._makeValueStrObj = function (edit_str, _is_fullvalue) {
		...
		var add_year = "", add_mon = "", add_day = "", add_time = "";
		...
		if (editformat_info) {
			if (editformat_info._year || editformat_info._mon || editformat_info._day) {
				...
				if (_is_fullvalue) {
					...
					if (edit_str.length < 9) {
						var empty_time = true;
						add_time = preval_str ? preval_str.substr(8) : "000000000";
					}
			...
			if (!this._isUndefinedDate(value_str) && !this._isNullDate(value_str) && !this._isEmptyStr(value_str)) {
				if (empty_date) {
					...
					if (empty_time && add_time) {
						if (value_str < 9) {
							value_str += add_time;
						}
						else {
							value_str = value_str.substr(0, 8) + add_time;
						}
					}
